...
void mainMenu(void)
{
	...
	eFabricantePantallas unaPantalla[NATIONSIZE] = {	{1, "FOXCONN"},
																								{2, "SHARP"},
																								{3, "JAPAN DISPLAY INC"}	};
	...
	do
	{
		...
		{
		case 1:
			if(newProduct(unProducto, unTipo, unaNacionalidad, unaPantalla, lastID, -1) == 0)
			{
		...
		case 7:
			...
			break;
		case 8:
			modificacionTipo(unProducto, unTipo, unaNacionalidad, unaPantalla, lastID);
			break;
		case 9:
			modificacionNacionalidad(unProducto, unTipo, unaNacionalidad, unaPantalla, lastID);
			break;
		case 10:
			if(count > 0)
			{
...
int newProduct(eProducto list[], TipoProducto tipo[], Nacionalidad nacion[], eFabricantePantallas pantalla[], int lastID, int aux)
{
	int retorno = -1;
	int n;
	...
	if(n != -1)
	{
		...
		list[n].idPantalla = screenMenu(pantalla);
...
int MenuInformes(void)
{
	...
	opcion = RequestIntInRange(	"Que desea saber.\n"
													...
													"10 - La nacionalidad con más tipos de productos fabricados.\n"
...
void modificacionTipo(eProducto list[], TipoProducto prod[], Nacionalidad nacion[], eFabricantePantallas pantalla[], int lastID)
{
	int opcion;
	int choice;
	int opcionBorrar;
	...
	do
	{
		...
		{
		case 1:
			newProduct(list, prod, nacion,  pantalla, lastID, opcion);
			break;
		case 2:
			productoPorTipo(list, prod, nacion, pantalla, opcion);
			break;
		case 3:
...
void productoPorTipo(eProducto list[], TipoProducto prod[], Nacionalidad nacion[], eFabricantePantallas pantalla[], int aux)
{
	int opcion;
		...
		if(list[i].isEmpty == OCUPADO && list[i].idProducto == opcion)
		{
			do
			{
				...
				case 3:
					...
					break;
				case 4:
					list[i].idPantalla = screenMenu(pantalla);
					break;
				}
...
int menuModificarTipo(void)
{
	...
	puts("Que desea modificar\n"
			...
			"5 - Volver sin modificar.\n");
	opcion = RequestIntInRange("Opcion; ", 1, 5);

	return opcion;
}

void modificacionNacionalidad(eProducto list[], TipoProducto prod[], Nacionalidad nacion[], eFabricantePantallas pantalla[], int lastID)
{
	int opcion;
	int choice;
	int opcionBorrar;
...
void productoPorNacionalidad(eProducto list[], TipoProducto prod[], Nacionalidad nacion[], eFabricantePantallas pantalla[], int aux)
{
	int opcion;
	int opcionModificacion = 0;
...
int screenMenu(eFabricantePantallas pantalla[])
{
	int opcion;
	puts("ID PANTALLA | DESCRIPCION PANTALLA\n");
	for(int i = 0; i < NATIONSIZE; i++)
	{
		printf("%d %10s\n", pantalla[i].idFabricante, pantalla[i].descripcionPantalla);
	}
	opcion = RequestIntInRange("Ingrese una opcion: ", 1, 3);
